...
typedef enum {
  ...
  CANDO_EXEC_ON_CHANGE = 2,   /* Fire only when payload/evaluated value
changes */
  CANDO_EXEC_POLL_VERIFY = 3, /* Fire action and wait for confirmation
status CAN message */
  CANDO_EXEC_TOGGLE = 4,      /* Flip-flop toggle mode
(Press to Start ON actions, Press to Cancel OFF actions) */
} cando_exec_mode_t;
...
typedef struct {
  ...
  char expression;
  /* Optional trigger math expression string */

  /* Hold Duration & Continuous Assertion */
  uint32_t for_ms; /* Must remain continuously active for this duration
                      before firing (0 = instant) */
  int64_t asserted_since_us; /* Timestamp when condition was first asserted (0 =
                                not asserted) */
  bool hold_fired; /* Flag indicating hold threshold has fired for current
  ...
  uint8_t click_count_target; /* Number of clicks required to trigger (1 =
                                 single press, 2 = double press, etc.) */
...
typedef enum {
  CANDO_ROLL_NONE = 0,
  CANDO_ROLL_SEQ3, /* Cycles 0x0F -> 0x1F -> 0x2F (e.g. E-GMP 0x2CF Climate
Temp) / CANDO_ROLL_BYTE_INC,    / Cycles 0x00 -> 0xFF / CANDO_ROLL_NIBBLE_INC /
Cycles low nibble 0x0 -> 0xF */
} cando_roll_mode_t;
...
uint32_t tx_can_id;     /* Response CAN ID */
  ...
  int8_t
      roll_byte_idx; /* -1 if no rolling byte, or 0..7 index of rolling byte */
...
typedef struct {
  ...
  char trigger_id[32];      /* Only execute if triggered by this trigger_id
                               (empty/NULL = any) */
  char popup_message;       /* Optional dashboard track popup text(via
                               track_popup_show) */
  ...
  cando_sequence_step_t steps; /* Sequence of CAN frames to play */
  uint8_t step_count;          /* Number of steps in the sequence */
...
typedef struct {
  ...
  bool ha_expose;      /* Expose as Home Assistant Button entity via MQTT Auto -
                          Discovery */
  ...
  uint32_t
      auto_revert_sec; /* Auto revert to OFF after N seconds (0 = disabled) */
...
typedef struct {
  ...
  /* Capture Mode : Auto, Always Paused, Disabled */
