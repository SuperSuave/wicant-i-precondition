...
static bool g_passenger_seatbelt_buckled = false; // 0x320 occupant sensor tracking
...
void cando_init(const char *device_id_str)
{
    ...
    if (g_cando_rules.mutex == NULL) {
        g_cando_rules.mutex = xSemaphoreCreateMutex();
    }

    cando_load_config();
    ESP_LOGI(TAG, "CAN Do Engine initialized (%lu rules loaded)", (unsigned long)g_cando_rules.rule_count);
}
...
static bool cando_evaluate_trigger(cando_trigger_t *trig, const twai_message_t *msg, uint8_t bus)
{
            ...
            if (end != NULL) {
                ...
                if (token_len < sizeof(token)) {
                    ...
                    if (strcasecmp(token, "battery_temp") == 0 || strcasecmp(token, "battery_temp_c") == 0 || strcasecmp(token, "temp") == 0) {
                        matched = true;
                        if (has_temp) snprintf(replacement, sizeof(replacement), "%d", (temp.min_c + temp.max_c) / 2);
                        else strncpy(replacement, "--", sizeof(replacement) - 1);
                    } else if (strcasecmp(token, "battery_temp_f") == 0 || strcasecmp(token, "temp_f") == 0) {
                        matched = true;
                        if (has_temp) snprintf(replacement, sizeof(replacement), "%d", (int)roundf(((temp.min_c + temp.max_c) / 2.0f * 9.0f / 5.0f) + 32.0f));
                        else strncpy(replacement, "--", sizeof(replacement) - 1);
                    } else if (strcasecmp(token, "voltage") == 0 || strcasecmp(token, "battery_voltage") == 0 || strcasecmp(token, "vbatt") == 0) {
                        matched = true;
                        if (has_volt) snprintf(replacement, sizeof(replacement), "%.1f", volt);
                        else strncpy(replacement, "--", sizeof(replacement) - 1);
                        ...
                        matched = true;
                        if (has_time) snprintf(replacement, sizeof(replacement), "%02d:%02d", tm_info.tm_hour, tm_info.tm_min);
                        else strncpy(replacement, "--:--", sizeof(replacement) - 1);
                    }
...
static void cando_execute_climate_target(float target_c, const char *zone, bool sync_on, bool driver_only, bool passenger_aware)
{
    // Passenger seatbelt occupant awareness check: if enabled and passenger seat is empty, force driver-only mode
    ...
    if (g_climate_has_reading) {
        ...
        if (cached > 0 && cached <= 36) cur_raw = cached;
    }

    int delta = (int)target_raw - (int)cur_raw;
    if (delta != 0) {
        uint8_t cmd_byte = (delta > 0) ? (is_passenger ? 0xD0 : 0x70) : (is_passenger ? 0xE0 : 0xB0);
...
static void cando_execute_action(cando_action_t *act, const char *matched_trig_id)
{
    ...
    if (act->type == CANDO_ACT_CLIMATE_TARGET) {
        cando_execute_climate_target(act->target_temp_c, act->climate_zone, act->climate_sync_on, act->climate_driver_only, act->climate_passenger_aware);
    }
...
static void cando_execute_rule_actions(cando_rule_t *rule, const char *matched_id, int64_t now_us)
{
    ...
    if (rule->exec_mode == CANDO_EXEC_TOGGLE) {
        if (!rule->is_active_state) {
            ...
            ESP_LOGI(TAG, "Rule '%s' TOGGLED ON", rule->name ? rule->name : "Unnamed");
            ...
            ESP_LOGI(TAG, "Rule '%s' TOGGLED OFF / CANCELLED", rule->name ? rule->name : "Unnamed");
...
{
    if (!msg) return;
    if (g_cando_rules.mutex && xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(10)) != pdTRUE) {
        return;
    }

    if (!g_cando_rules.rules || g_cando_rules.rule_count == 0) {
        if (g_cando_rules.mutex) xSemaphoreGive(g_cando_rules.mutex);
        return;
    }
    if (cando_is_capture_active()) {
        if (g_cando_rules.mutex) xSemaphoreGive(g_cando_rules.mutex);
        return;
    }
    ...
    for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
        ...
        for (uint8_t t_idx = 0; t_idx < t_count; t_idx++) {
            ...
            bool payload_changed = !trig->has_last_payload || (memcmp(trig->last_payload, msg->data, msg->data_length_code) != 0);
            ...
            if (eval_passed) {
                ...
                if (trig->click_count_target > 1) continue;
                ...
                if (trig->for_ms > 0) {
                    ...
                    if (elapsed_ms < trig->for_ms) continue;
                    if (trig->hold_fired) continue;
                ...
                if (trig->exec_mode == CANDO_EXEC_ONE_SHOT) {
                    if (trig->triggered_latched) continue;
                    trig->triggered_latched = true;
                }

                if (trig->exec_mode == CANDO_EXEC_POLL_VERIFY) {
                    if (trig->triggered_latched || trig->pending_verify) continue;
                ...
                if (rule->trigger_combine_all && t_count > 1) {
                    for (uint8_t k = 0; k < t_count; k++) {
                        ...
                        if (trig_list[k].exec_mode == CANDO_EXEC_ONE_SHOT) trig_list[k].triggered_latched = true;
                    }
                }
                ...
                if (rule->trigger_combine_all && t_count > 1) break;
            }
        }
    }
    ...
    for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
        ...
        for (uint8_t t_idx = 0; t_idx < t_count; t_idx++) {
            ...
            if (trig->mqtt_topic[0] != '\0' && strcmp(trig->mqtt_topic, "#") != 0 && strcmp(trig->mqtt_topic, "*") != 0) {
                if (strstr(topic, trig->mqtt_topic) == NULL && strcmp(topic, trig->mqtt_topic) != 0) continue;
            }

            if (trig->mqtt_payload[0] != '\0' && strcmp(trig->mqtt_payload, "*") != 0) {
                if (!payload || (strstr(payload, trig->mqtt_payload) == NULL && strcmp(payload, trig->mqtt_payload) != 0)) continue;
            }

            uint32_t cd_ms = (trig->cooldown_ms > 0) ? trig->cooldown_ms : 50;
            if (trig->last_triggered_us > 0 && (now_us - trig->last_triggered_us) < ((int64_t)cd_ms * 1000)) continue;
...
static void cando_parse_single_trigger(cJSON *r, cJSON *trig_obj, cando_trigger_t *trig)
{
    ...
    if (cc && cJSON_IsNumber(cc) && cc->valueint >= 1) trig->click_count_target = (uint8_t)cc->valueint;

    cJSON *cw = trig_obj ? cJSON_GetObjectItem(trig_obj, "click_window_ms") : cJSON_GetObjectItem(r, "click_window_ms");
    if (cw && cJSON_IsNumber(cw) && cw->valueint > 0) trig->click_window_ms = (uint32_t)cw->valueint;

    cJSON *tid = trig_obj ? cJSON_GetObjectItem(trig_obj, "id") : NULL;
    if (tid && tid->valuestring) strncpy(trig->id, tid->valuestring, sizeof(trig->id) - 1);
    ...
    if (f_ms && cJSON_IsNumber(f_ms)) trig->for_ms = (uint32_t)f_ms->valueint;
    else if (f_sec && cJSON_IsNumber(f_sec)) trig->for_ms = (uint32_t)roundf(f_sec->valuedouble * 1000.0f);
    ...
    if (rcid && rcid->valuestring && strlen(rcid->valuestring) > 0) trig->reset_can_id = strtoul(rcid->valuestring, NULL, 0);

    cJSON *vcid = trig_obj ? cJSON_GetObjectItem(trig_obj, "verify_can_id") : cJSON_GetObjectItem(r, "verify_can_id");
    if (vcid && vcid->valuestring && strlen(vcid->valuestring) > 0) trig->verify_can_id = strtoul(vcid->valuestring, NULL, 0);
    ...
    if (vp && vp->valuestring && strlen(vp->valuestring) > 0) {
        cando_parse_payload_pattern(vp->valuestring, trig->verify_data, trig->verify_mask, &trig->verify_len, &trig->has_verify);
    }
    ...
    if (bus_item && cJSON_IsNumber(bus_item)) trig->bus = (uint8_t)bus_item->valueint;

    if (from_p && from_p->valuestring && strlen(from_p->valuestring) > 0) {
        cando_parse_payload_pattern(from_p->valuestring, trig->from_data, trig->from_mask, &trig->from_len, &trig->has_from);
    }

    if (to_p && to_p->valuestring && strlen(to_p->valuestring) > 0) {
        trig->match_type = CANDO_MATCH_MASK;
        cando_parse_payload_pattern(to_p->valuestring, trig->match_data, trig->match_mask, &trig->data_len, &trig->has_to);
    }
    ...
    if (m_topic && m_topic->valuestring) strncpy(trig->mqtt_topic, m_topic->valuestring, sizeof(trig->mqtt_topic) - 1);

    cJSON *m_payload = trig_obj ? cJSON_GetObjectItem(trig_obj, "mqtt_payload") : cJSON_GetObjectItem(r, "mqtt_payload");
    if (m_payload && m_payload->valuestring) strncpy(trig->mqtt_payload, m_payload->valuestring, sizeof(trig->mqtt_payload) - 1);

    if (!trig->has_from && !trig->has_to) trig->any_change = true;
}
...
static void cando_parse_single_action(cJSON *r, cJSON *act_obj, cando_action_t *act)
{
    ...
    if (act_type_obj && act_type_obj->valuestring) {
        if (strcmp(act_type_obj->valuestring, "can_tx") == 0) act->type = CANDO_ACT_CAN_TX;
        else if (strcmp(act_type_obj->valuestring, "popup") == 0) act->type = CANDO_ACT_POPUP;
        else if (strcmp(act_type_obj->valuestring, "precondition") == 0) act->type = CANDO_ACT_PRECONDITION;
        else if (strcmp(act_type_obj->valuestring, "climate_target") == 0) act->type = CANDO_ACT_CLIMATE_TARGET;
        else if (strcmp(act_type_obj->valuestring, "delay") == 0) act->type = CANDO_ACT_DELAY;
        else if (strcmp(act_type_obj->valuestring, "mqtt") == 0) act->type = CANDO_ACT_MQTT;
        else if (strcmp(act_type_obj->valuestring, "webhook") == 0) act->type = CANDO_ACT_WEBHOOK;
        else act->type = CANDO_ACT_CAN_TX;
    } else {
        act->type = CANDO_ACT_CAN_TX;
    }

    cJSON *tt = act_obj ? cJSON_GetObjectItem(act_obj, "target_temp_c") : cJSON_GetObjectItem(r, "target_temp_c");
    act->target_temp_c = (tt && cJSON_IsNumber(tt)) ? (float)tt->valuedouble : 21.0f;

    cJSON *cz = act_obj ? cJSON_GetObjectItem(act_obj, "climate_zone") : cJSON_GetObjectItem(r, "climate_zone");
    if (cz && cz->valuestring) strncpy(act->climate_zone, cz->valuestring, sizeof(act->climate_zone) - 1);
    else strcpy(act->climate_zone, "driver");
    ...
    cJSON *pass_aware_obj = act_obj ? cJSON_GetObjectItem(act_obj, "climate_passenger_aware") : cJSON_GetObjectItem(r, "climate_passenger_aware");
    act->climate_passenger_aware = (pass_aware_obj && cJSON_IsTrue(pass_aware_obj));
    ...
    if (pop && pop->valuestring && strlen(pop->valuestring) > 0) act->popup_message = strdup(pop->valuestring);

    cJSON *pm = act_obj ? cJSON_GetObjectItem(act_obj, "precon_mode") : cJSON_GetObjectItem(r, "precon_mode");
    if (pm && pm->valuestring) strncpy(act->precon_mode, pm->valuestring, sizeof(act->precon_mode) - 1);

    cJSON *pp = act_obj ? cJSON_GetObjectItem(act_obj, "precon_press") : cJSON_GetObjectItem(r, "precon_press");
    if (pp && pp->valuestring) strncpy(act->precon_press, pp->valuestring, sizeof(act->precon_press) - 1);
    ...
    if (act_bus && cJSON_IsNumber(act_bus)) target_bus = (uint8_t)act_bus->valueint;
    if (act_delay && cJSON_IsNumber(act_delay)) delay_val = (uint32_t)act_delay->valueint;

    cJSON *w_ms = act_obj ? cJSON_GetObjectItem(act_obj, "wait_ms") : NULL;
    if (!w_ms && act_obj) w_ms = cJSON_GetObjectItem(act_obj, "delay_ms");
    if (!w_ms) w_ms = cJSON_GetObjectItem(r, "wait_ms");
    if (!w_ms) w_ms = cJSON_GetObjectItem(r, "delay_ms");
...
{
    if (g_cando_rules.mutex && xSemaphoreTake(g_cando_rules.mutex, portMAX_DELAY) != pdTRUE) {
        ...
        if (count > 0) {
            ...
            if (g_cando_rules.rules) {
                for (int i = 0; i < count; i++) {
                    ...
                    if (en && cJSON_IsBool(en)) rule->enabled = cJSON_IsTrue(en);

                    cJSON *name = cJSON_GetObjectItem(r, "name");
                    if (name && name->valuestring) rule->name = strdup(name->valuestring);

                    cJSON *ha_exp = cJSON_GetObjectItem(r, "ha_expose");
                    if (ha_exp && cJSON_IsBool(ha_exp)) rule->ha_expose = cJSON_IsTrue(en);

                    cJSON *ha_ic = cJSON_GetObjectItem(r, "ha_icon");
                    if (ha_ic && ha_ic->valuestring) strncpy(rule->ha_icon, ha_ic->valuestring, sizeof(rule->ha_icon) - 1);
                    ...
                    if (rev_sec && cJSON_IsNumber(rev_sec)) rule->auto_revert_sec = (uint32_t)rev_sec->valueint;
                    ...
                    if (trig_mode && trig_mode->valuestring) {
                        rule->trigger_combine_all = (strcmp(trig_mode->valuestring, "all") == 0 ||
                    ...
                    if (off_acts_arr && cJSON_IsArray(off_acts_arr) && cJSON_GetArraySize(off_acts_arr) > 0) {
                        ...
                        if (off_act_obj) cando_parse_single_action(r, off_act_obj, &rule->off_action);
                    }
...
{
    if (g_cando_rules.mutex && xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(50)) != pdTRUE) {
...
bool cando_test_single_action_json(const char *json_str)
{
    ...
    if (act.steps) free(act.steps);
    if (act.popup_message) free(act.popup_message);
...
void cando_get_stats_json(cJSON *root)
{
    ...
    if (g_cando_rules.mutex && xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(20)) != pdTRUE) {
        return;
    }
    ...
    for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
        ...
        for (uint8_t t = 0; t < t_count; t++) {
            if (trig_list[t].source == CANDO_TRIG_MQTT_COMMAND) {
                should_expose = true;
                if (trig_list[t].mqtt_payload[0] != '\0') trigger_payload = trig_list[t].mqtt_payload;
                break;
            }
        }
        ...
        if (json_str) {
            ...
            ESP_LOGI(TAG, "Published HA Button discovery for '%s' to %s", rule->name, disc_topic);
            free(json_str);
        }
        cJSON_Delete(root);
...
void cando_unpublish_ha_rule(const char *rule_name)
{
    ...
    ESP_LOGI(TAG, "Unpublished HA Button discovery for '%s' (%s)", rule_name, disc_topic);
}