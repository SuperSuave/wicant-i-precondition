...
static void wifi_network_event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (wifi_init && event_base == WIFI_EVENT &&
    ...
    if (err == ESP_OK && current_mode == WIFI_MODE_STA &&
        !sta_successfuly_connected && ap_auto_disable && wifi_init) {
...
void wifi_network_deinit(void) {
  xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECT_IDLE_BIT, pdFALSE,
...
static void wifi_conn_task(void *pvParameters) {
  while (1) {
    xEventGroupWaitBits(s_wifi_event_group,
                        WIFI_INIT_BIT | WIFI_DISCONNECTED_BIT, pdFALSE, pdTRUE,
                        portMAX_DELAY);
    ...
    if (net_count > 1) {
      ...
      if (net && strlen(net->ssid) > 0) {
        ...
        strncpy((char *)wifi_cfg.sta.ssid, config_server_get_sta_ssid(),
                sizeof(wifi_cfg.sta.ssid) - 1);
        strncpy((char *)wifi_cfg.sta.password, config_server_get_sta_pass(),
                sizeof(wifi_cfg.sta.password) - 1);
    ...
    if ((bits & WIFI_CONNECT_IDLE_BIT) == 0) {
      ESP_LOGW(WIFI_TAG,
               "Wi-Fi connection attempt timed out waiting for event");
      esp_wifi_disconnect();
      xEventGroupSetBits(s_wifi_event_group, WIFI_DISCONNECTED_BIT);
      xEventGroupSetBits(s_wifi_event_group, WIFI_CONNECT_IDLE_BIT);
    }
...
void wifi_network_init(char *sta_ssid, char *sta_pass) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_instance_register(
      WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_network_event_handler, NULL,
      &instance_any_id));
  ESP_ERROR_CHECK(esp_event_handler_instance_register(
      IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_network_event_handler, NULL,
      &instance_got_ip));
  ...
  static wifi_config_t wifi_config_ap = {
      .ap = {.max_connection = 4, .authmode = WIFI_AUTH_WPA2_WPA3_PSK},
  };
  wifi_config_ap.ap.channel = channel;

  if (config_server_get_wifi_mode() == APSTA_MODE ||
      (sta_ssid != 0 && sta_pass != 0)) {
    if (sta_ssid == 0 && sta_pass == 0) {
      strcpy((char *)wifi_config_sta.sta.ssid,
             (char *)config_server_get_sta_ssid());
      strcpy((char *)wifi_config_sta.sta.password,
             (char *)config_server_get_sta_pass());
    } else {
    ...
    if (xwifi_handle == NULL) {
      xTaskCreate(wifi_conn_task, "wifi_conn_task", 4096, (void *)AF_INET, 5,
                  &xwifi_handle);
    }
  } else {
  ...
  strcpy((char *)wifi_config_ap.ap.password,
         (char *)config_server_get_ap_pass());

  esp_err_t hostname_err =
      esp_netif_set_hostname(sta_netif, (char *)wifi_config_ap.ap.ssid);
  if (hostname_err == ESP_OK) {
    ESP_LOGI(WIFI_TAG, "Hostname set to: %s", (char *)wifi_config_ap.ap.ssid);
  } else {
    ESP_LOGE(WIFI_TAG, "Failed to set hostname: %s",
