...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_ERROR:
    ...
    //            event->error_handle->esp_transport_sock_errno); ESP_LOGI(TAG,
    //            "Last errno string (%s)",
...
/// format:{"bus":"0","type":"rx","ts":34610,"frame":[{"id":123,"dlc":8,"rtr":false,"extd":false,"data":[1,2,3,4,5,6,7,8]},{"id":124,"dlc":8,"rtr":false,"extd":true,"data":[1,2,3,4,5,6,7,8]}]}
...
static void mqtt_parse_data(void *handler_args, esp_event_base_t base,
  ...
  if ((mqtt_elm327_log == 0) &&
      strncmp(event->topic, mqtt_sub_topic, strlen(mqtt_sub_topic)) == 0) {
    ...
    for (int i = 0; i < cJSON_GetArraySize(frame_ary); i++) {
      ...
      can_frame.identifier = cJSON_IsTrue(frame_extd)
                                 ? (frame_id->valueint & TWAI_EXTD_ID_MASK)
                                 : (frame_id->valueint & TWAI_STD_ID_MASK);
      can_frame.data_length_code =
          (frame_dlc->valueint > 8) ? 8 : frame_dlc->valueint;
      can_frame.extd = cJSON_IsTrue(frame_extd) ? 1 : 0;
      can_frame.rtr = cJSON_IsTrue(frame_rtr) ? 1 : 0;

      // ESP_LOGI(TAG, "frame_id: %d, frame_dlc: %d", can_frame.identifier,
      // can_frame.data_length_code);
    ...
    int len = (event->topic_len < (sizeof(topic_buf) - 1))
                  ? event->topic_len
                  : (sizeof(topic_buf) - 1);
...
static void mqtt_task(void *pvParameters) {
  ...
  esp_mqtt_client_register_event(client, ESP_EVENT_ANY_ID, mqtt_event_handler,
                                 NULL);
  esp_mqtt_client_register_event(client, MQTT_EVENT_DATA, mqtt_parse_data,
                                 NULL);
    ...
    if (mqtt_connected()) {
      ...
      if (tx_frame.type == MQTT_CAN) {
        if (mqtt_canflt_size != 0) {
          ...
          while ((found_index = mqtt_canflt_find_id(tx_frame.frame.identifier,
                                                    start_index)) != -1) {

            // Check if expecting PID
            if (mqtt_canflt_values[found_index].pid != -1) {
              if (mqtt_canflt_values[found_index].pid !=
                  tx_frame.frame.data[mqtt_canflt_values[found_index].pidi]) {
                start_index = found_index + 1;
                continue;
              }
            }

            if (esp_timer_get_time() - mqtt_canflt_values[found_index].logtime <
                (mqtt_canflt_values[found_index].cycle * 1000)) {
              break;
            }
            ...
            uint64_t start_bit = 64 -
                                 mqtt_canflt_values[found_index].start_bit -
                                 mqtt_canflt_values[found_index].bit_length;
            ...
            if (evaluate_expression(
                    (uint8_t *)mqtt_canflt_values[found_index].expression,
                    (uint8_t *)tx_frame.frame.data, (double)value,
                    &expression_result)) {
              ESP_LOGI(TAG, "Expression result: %lf", expression_result);

              sprintf(json_buffer, "{\"%s\": %lf}",
                      mqtt_canflt_values[found_index].name, expression_result);
          ...
          sprintf(json_buffer,
                  "{\"bus\":\"0\",\"type\":\"rx\",\"ts\":%lu,\"frame\":[",
                  (pdTICKS_TO_MS(xTaskGetTickCount()) % 60000));

          if (strlen(json_buffer) < (sizeof(json_buffer) - 128)) {
            while (xQueuePeek(*xmqtt_tx_queue, (void *)&tx_frame, 0) ==
                   pdTRUE) {
              xQueueReceive(*xmqtt_tx_queue, (void *)&tx_frame, 0);
              sprintf(tmp,
                      "{\"id\":%lu,\"dlc\":%u,\"rtr\":%s,\"extd\":%s,\"data\":["
                      "%u,%u,%u,%u,%u,%u,%u,%u]},",
                      tx_frame.frame.identifier,
                      tx_frame.frame.data_length_code,
                      tx_frame.frame.rtr ? "true" : "false",
                      tx_frame.frame.extd ? "true" : "false",
                      tx_frame.frame.data[0], tx_frame.frame.data[1],
                      tx_frame.frame.data[2], tx_frame.frame.data[3],
                      tx_frame.frame.data[4], tx_frame.frame.data[5],
                      tx_frame.frame.data[6], tx_frame.frame.data[7]);
        ...
        if (tx_frame.type == MQTT_RX) {
          ...
          sprintf(json_buffer,
                  "{\"bus\":\"0\",\"type\":\"tx\",\"ts\":%lu,\"frame\":[",
                  (pdTICKS_TO_MS(xTaskGetTickCount()) % 60000));
          ESP_LOGI(TAG, "tx_frame.type: MQTT_TX");
        }

        sprintf(tmp,
                "{\"id\":%lu,\"dlc\":%u,\"rtr\":%s,\"extd\":%s,\"data\":[%u,%u,"
                "%u,%u,%u,%u,%u,%u]},",
                tx_frame.frame.identifier, tx_frame.frame.data_length_code,
                tx_frame.frame.rtr ? "true" : "false",
                tx_frame.frame.extd ? "true" : "false", tx_frame.frame.data[0],
                tx_frame.frame.data[1], tx_frame.frame.data[2],
                tx_frame.frame.data[3], tx_frame.frame.data[4],
                tx_frame.frame.data[5], tx_frame.frame.data[6],
                tx_frame.frame.data[7]);
        strcat((char *)json_buffer, (char *)tmp);
        json_buffer[strlen(json_buffer) - 1] = 0;
        strcat((char *)json_buffer, "]}");
...
static void mqtt_load_filter(void) {
  ...
  mqtt_canflt_values =
      (CANFilter *)malloc(mqtt_canflt_size * sizeof(CANFilter));
  ...
  for (uint32_t i = 0; i < mqtt_canflt_size; i++) {
    ...
    if (cJSON_IsNumber(can_id) && cJSON_IsString(name) && cJSON_IsNumber(pid) &&
        cJSON_IsNumber(pidi) && cJSON_IsNumber(start_bit) &&
        cJSON_IsNumber(bit_length) && cJSON_IsString(expression) &&
        cJSON_IsNumber(cycle)) {
      mqtt_canflt_values[i].can_id = (uint32_t)can_id->valuedouble;
      strncpy(mqtt_canflt_values[i].name, name->valuestring,
              sizeof(mqtt_canflt_values[i].name));
      ...
      strncpy(mqtt_canflt_values[i].expression, expression->valuestring,
              sizeof(mqtt_canflt_values[i].expression));
      mqtt_canflt_values[i].cycle = (uint32_t)cycle->valuedouble;
      mqtt_canflt_values[i].logtime = 0;

      ESP_LOGI(
          TAG,
          "Loaded CAN Filter %lu: CAN ID=%lu, PID=%ld, PIDIndex=%ld, Name=%s, "
          "Start Bit=%lu, Bit Length=%lu, Expression=%s, Cycle=%lu",
          i, mqtt_canflt_values[i].can_id, mqtt_canflt_values[i].pid,
          mqtt_canflt_values[i].pidi, mqtt_canflt_values[i].name,
...
void mqtt_publish(char *topic, char *data, int len, int qos, int retain) {
  ...
  if ((config_server_mqtt_en_config() == 1) && mqtt_connected() &&
      (xSemaphoreTake(xmqtt_semaphore, pdMS_TO_TICKS(2000)) == pdTRUE)) {
    if (len == 0) {
    ...
    if (data_len < MQTT_TX_RX_BUF_SIZE) {
      ...
      static const char *error_msg =
          "{\"error\": \"Data length exceeds the data size\"}";
      esp_mqtt_client_publish(client, topic, error_msg, strlen(error_msg), qos,
                              0);
    }
