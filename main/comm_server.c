...
wait_skt_rx:
  ...
  //    int len = recvfrom(listen_sock, rx_buffer, sizeof(rx_buffer) - 1, 0,
  //    (struct sockaddr *)&source_addr, &socklen);
  while (1) {
    //		 = recv(sock, rx_buffer.ucElement, sizeof(rx_buffer.ucElement) -
    //1, 0);
    rx_buffer.usLen = recvfrom(listen_sock, rx_buffer.ucElement,
                               sizeof(rx_buffer.ucElement) - 1, 0,
                               (struct sockaddr *)&source_addr, &socklen);
    if (rx_buffer.usLen < 0) {
      xEventGroupSetBits(xSocketEventGroup, PORT_CLOSED_BIT);
      xEventGroupClearBits(xSocketEventGroup, PORT_OPEN_BIT);
...
wait_skt_tx:
  ...
  while (1) {
    ...
    //		ESP_LOGI(TAG, "Sending %d bytes: %s", tx_buffer.usLen,
    //tx_buffer.ucElement);
    if (xSemaphoreTake(xTCP_Socket_Semaphore, portMAX_DELAY) == pdTRUE) {
      int err = sendto(listen_sock, tx_buffer.ucElement, tx_buffer.usLen, 0,
...
static void tcp_server_tx_task(void *pvParameters) {
  ...
  while (1) {
    ...
    if (xSemaphoreTake(xTCP_Socket_Semaphore, portMAX_DELAY) == pdTRUE) {
      ...
      while (to_write > 0) {
        int written =
            send(sock, coalesce_buf + (bytes_used - to_write), to_write, 0);
        if (written < 0) {
          ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
...
static void tcp_server_task(void *pvParameters) {
  ...
  while (1) {
    ...
    if (!udp_enable) {
      ...
      setsockopt(new_sock, IPPROTO_TCP, TCP_KEEPINTVL, &keepInterval,
                 sizeof(int));
      setsockopt(new_sock, IPPROTO_TCP, TCP_KEEPCNT, &keepCount, sizeof(int));
      // Bound how many GVRET commands can queue on-device, so keepalive replies
      // stay within SavvyCAN's ~10 s watchdog even when a CAN bus is saturated.
      ...
      if (sock >= 0 &&
          (xEventGroupGetBits(xSocketEventGroup) & PORT_OPEN_BIT)) {
        fd_set rfds;
        ...
        if (select(new_sock + 1, &rfds, NULL, NULL, &tv) <= 0 ||
          ...
          ESP_LOGW(TAG,
                   "Rejecting %s: session active and newcomer sent no data",
                   addr_str);
          close(new_sock);
          continue;
        }
        ESP_LOGW(TAG, "New client %s, closing previous session; hello bytes:",
                 addr_str);
      ...
      if (sock >= 0) {
        ...
        if (!(xEventGroupWaitBits(xSocketEventGroup, PORT_CLOSED_BIT, pdFALSE,
                                  pdFALSE, pdMS_TO_TICKS(5000)) &
              PORT_CLOSED_BIT)) {
          ESP_LOGE(TAG, "Previous session did not release the socket, "
      ...
      // Data received
        ...
        int err = sendto(listen_sock, rx_buffer, len, 0,
...
int8_t tcp_server_init(uint32_t port, QueueHandle_t *xTXp_Queue,
  ...
  xTaskCreate(tcp_server_task, "tcp_server", 4096, (void *)AF_INET, 5,
              &xserver_handle);
  if (!udp_enable) {
    xTaskCreate(tcp_server_rx_task, "tcp_rx_server", 4096, (void *)AF_INET, 5,
                &xrx_handle);
    xTaskCreate(tcp_server_tx_task, "tcp_tx_server", 4096, (void *)AF_INET, 5,
                &xtx_handle);
  } else {
    xTaskCreate(udp_server_rx_task, "udp_rx_server", 4096, (void *)AF_INET, 5,
                &xrx_handle);
    xTaskCreate(udp_server_tx_task, "udp_tx_server", 4096, (void *)AF_INET, 5,
                &xtx_handle);
  }
  return 0;
}
