...
#define DEFAULT_CANDO_JSON                                                     \
  ...
  ":" E -                                                                      \
      GMP Battery Preconditioning                                              \
          "," enabled ":true," trigger ":{" id ":" sw_star "," source          \
          ":" can_message "," can_id ":" 0x448 "," bus ":0," exec_mode         \
                                               ":" one_shot "," cooldown_ms    \
                                               ":500," from ":" *              \
          ****0 * "," to ":" * ****1 *                                         \
          "}," action ":{" type ":" precondition "," trigger_id ":" sw_star    \
          "," precon_mode ":" persistent "," precon_press ":" short "}}]}"

static cando_rule_set_t g_cando_rules = {0};
static char g_device_id[32] = {0};

/* Cache for E-GMP 0x380 live cabin temperatures */
...
void cando_init(const char *device_id_str) {
  ...
  ESP_LOGI(TAG, "CAN Do Engine initialized (%lu rules loaded)",
           (unsigned long)g_cando_rules.rule_count);
}

static bool cando_parse_payload_pattern(const char *str, uint8_t *data,
...
static bool cando_evaluate_trigger(cando_trigger_t *trig,
...
static void cando_execute_climate_target(float target_c, const char *zone,
  ...
  static void cando_execute_action(cando_action_t * act,
    ...
    if (act->popup_message && act->popup_message[0] != '\0') {
      ...
      cando_format_popup_message(act->popup_message, formatted_msg,
    ...
    if (act->type == CANDO_ACT_CLIMATE_TARGET) {
      cando_execute_climate_target(act->target_temp_c, act->climate_zone,
                                   act->climate_sync_on,
                                   act->climate_driver_only);
    }

    if (act->type == CANDO_ACT_DELAY) {
      uint32_t ms = (act->delay_ms > 0) ? act->delay_ms : 500;
      ESP_LOGI(TAG, "Executing Delay Action: pausing %lu ms",
               (unsigned long)ms);
  ...
  static void cando_execute_rule_actions(
      cando_rule_t * rule, const char *matched_id, int64_t now_us) {
    if (!rule)
      return;
    if (rule->exec_mode == CANDO_EXEC_TOGGLE) {
      if (!rule->is_active_state) {
        ...
        } else if (rule->off_action.type != 0 ||
                   rule->off_action.popup_message ||
  ...
  bool cando_evaluate_rule(cando_rule_t * rule, const twai_message_t *msg,
    ...
    if (!rule || !rule->enabled)
      return false;
    if (g_cando_rules.reverse_engineering_mode)
      return false;
    if (rule->trigger_count > 0 && rule->triggers) {
      for (uint8_t i = 0; i < rule->trigger_count; i++) {
        if (cando_evaluate_trigger(&rule->triggers[i], msg, bus))
          return true;
      }
      return false;
    }
  ...
  void cando_process_rx_frame(const twai_message_t *msg, uint8_t bus) {
    if (!msg)
      return;
    if (g_cando_rules.mutex &&
        xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(10)) != pdTRUE) {
      return;
    }

    if (!g_cando_rules.rules || g_cando_rules.rule_count == 0) {
      if (g_cando_rules.mutex)
        xSemaphoreGive(g_cando_rules.mutex);
      return;
    }
    ...
    for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
      ...
      cando_trigger_t *trig_list = (rule->trigger_count > 0 && rule->triggers)
                                       ? rule->triggers
                                       : &rule->trigger;
      ...
      for (uint8_t t_idx = 0; t_idx < t_count; t_idx++) {
        ...
        if (trig->exec_mode == CANDO_EXEC_POLL_VERIFY && trig->pending_verify) {
          if (trig->verify_can_id > 0 &&
              msg->identifier == trig->verify_can_id) {
            bool verify_ok = true;
            if (trig->has_verify) {
          ...
          if (is_release_state) {
            ...
            if (trig->click_count_target > 1 && trig->was_pressed) {
              ...
              if (trig->current_clicks >= trig->click_count_target) {
                ...
                uint32_t cd_ms =
                    (trig->cooldown_ms > 0) ? trig->cooldown_ms : 50;
  ...
  static void cando_parse_single_trigger(cJSON * r, cJSON * trig_obj,
                                         cando_trigger_t * trig) {
    ...
    cJSON *t_reset = trig_obj
                         ? cJSON_GetObjectItem(trig_obj, "timeout_reset_ms")
    ...
    if (vp && vp->valuestring && strlen(vp->valuestring) > 0) {
      cando_parse_payload_pattern(vp->valuestring, trig->verify_data,
    ...
    if (from_p && from_p->valuestring && strlen(from_p->valuestring) > 0) {
      cando_parse_payload_pattern(from_p->valuestring, trig->from_data,
    ...
    if (to_p && to_p->valuestring && strlen(to_p->valuestring) > 0) {
      ...
      cando_parse_payload_pattern(to_p->valuestring, trig->match_data,
  ...
  static void cando_parse_step_payload(const char *payload_str,
                                       cando_sequence_step_t *step) {
    if (!step)
      return;
  ...
  static void cando_parse_single_action(cJSON * r, cJSON * act_obj,
                                        cando_action_t * act) {
    if (!act)
      return;
    ...
    if (trig_id && trig_id->valuestring) {
      strncpy(act->trigger_id, trig_id->valuestring,
              sizeof(act->trigger_id) - 1);
    } else {
    ...
    if (cz && cz->valuestring)
      strncpy(act->climate_zone, cz->valuestring,
              sizeof(act->climate_zone) - 1);
    else
      strcpy(act->climate_zone, "driver");

    cJSON *sync_on_obj = act_obj
                             ? cJSON_GetObjectItem(act_obj, "climate_sync_on")
    ...
    if (pp && pp->valuestring)
      strncpy(act->precon_press, pp->valuestring,
              sizeof(act->precon_press) - 1);
    ...
    cJSON *act_delay =
        act_obj ? cJSON_GetObjectItem(act_obj, "delay_ms") : NULL;
  ...
  static void cando_init_default_precondition_rule(void) {
    if (g_cando_rules.rule_count > 0 && g_cando_rules.rules)
      return;
  ...
  esp_err_t cando_load_config(void) {
    if (g_cando_rules.mutex &&
        xSemaphoreTake(g_cando_rules.mutex, portMAX_DELAY) != pdTRUE) {
    ...
    if (rules_arr && cJSON_IsArray(rules_arr)) {
      ...
      if (g_cando_rules.rules) {
        for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
          if (g_cando_rules.rules[i].name)
            free(g_cando_rules.rules[i].name);
          if (g_cando_rules.rules[i].triggers)
            free(g_cando_rules.rules[i].triggers);
          if (g_cando_rules.rules[i].actions) {
            for (uint8_t a = 0; a < g_cando_rules.rules[i].action_count; a++) {
              if (g_cando_rules.rules[i].actions[a].popup_message)
                free(g_cando_rules.rules[i].actions[a].popup_message);
              if (g_cando_rules.rules[i].actions[a].steps)
                free(g_cando_rules.rules[i].actions[a].steps);
            }
            free(g_cando_rules.rules[i].actions);
          } else {
            if (g_cando_rules.rules[i].action.popup_message)
              free(g_cando_rules.rules[i].action.popup_message);
            if (g_cando_rules.rules[i].action.steps)
              free(g_cando_rules.rules[i].action.steps);
          }
          if (g_cando_rules.rules[i].off_actions) {
            for (uint8_t a = 0; a < g_cando_rules.rules[i].off_action_count;
                 a++) {
              if (g_cando_rules.rules[i].off_actions[a].popup_message)
                free(g_cando_rules.rules[i].off_actions[a].popup_message);
              if (g_cando_rules.rules[i].off_actions[a].steps)
                free(g_cando_rules.rules[i].off_actions[a].steps);
            }
            free(g_cando_rules.rules[i].off_actions);
          } else {
            if (g_cando_rules.rules[i].off_action.popup_message)
              free(g_cando_rules.rules[i].off_action.popup_message);
            if (g_cando_rules.rules[i].off_action.steps)
              free(g_cando_rules.rules[i].off_action.steps);
          }
        }
  ...
  char *cando_get_config(void) {
    if (g_cando_rules.mutex &&
        xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(50)) != pdTRUE) {
  ...
  void cando_get_stats_json(cJSON * root) {
    ...
    if (g_cando_rules.mutex &&
        xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(20)) != pdTRUE) {
      return;
    }
    ...
    for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
      ...
      cJSON_AddStringToObject(
          st, "name",
          g_cando_rules.rules[i].name ? g_cando_rules.rules[i].name : "");
      cJSON_AddNumberToObject(st, "count", g_cando_rules.rules[i].exec_count);
      cJSON_AddBoolToObject(st, "is_active",
                            g_cando_rules.rules[i].is_active_state);
      int64_t age_ms =
          (g_cando_rules.rules[i].last_exec_us > 0)
              ? (now_us - g_cando_rules.rules[i].last_exec_us) / 1000
              : -1;
    ...
    cJSON_AddNumberToObject(root, "capture_mode",
                            (int)g_cando_rules.capture_mode);
  ...
  void cando_set_capture_mode(cando_capture_mode_t mode) {
    ...
    g_cando_rules.reverse_engineering_mode =
  ...
  bool cando_is_capture_active(void) {
    if (g_cando_rules.capture_mode == CANDO_CAPTURE_ALWAYS_PAUSED ||
        g_cando_rules.reverse_engineering_mode) {
      return true;
    }
  ...
  void cando_set_reverse_engineering_mode(bool enable) {
    cando_set_capture_mode(enable ? CANDO_CAPTURE_ALWAYS_PAUSED
                                  : CANDO_CAPTURE_AUTO);
  }
  ...
  void cando_publish_ha_discovery(void) {
    if (!mqtt_connected() || !g_cando_rules.rules ||
        g_cando_rules.rule_count == 0)
      return;
    if (g_cando_rules.mutex &&
        xSemaphoreTake(g_cando_rules.mutex, pdMS_TO_TICKS(50)) != pdTRUE) {
    ...
    for (uint32_t i = 0; i < g_cando_rules.rule_count; i++) {
      ...
      sanitize_ha_identifier(rule->name, sanitized_name,
                             sizeof(sanitized_name));

      char disc_topic[192];
      snprintf(disc_topic, sizeof(disc_topic),
               "homeassistant/button/wican_%s/cando_%s/config", dev_id,
               sanitized_name);
      ...
      char uniq_id[160];
      snprintf(uniq_id, sizeof(uniq_id), "wican_%s_cando_%s", dev_id,
               sanitized_name);
      ...
      if (json_str) {
        ...
        ESP_LOGI(TAG, "Published HA Button discovery for '%s' to %s",
                 rule->name, disc_topic);
        free(json_str);
      }
      cJSON_Delete(root);
