...
static esp_err_t _http_event_handle(esp_http_client_event_t *evt) {
  // Can be used to capture response data chunks if needed
  return ESP_OK;
}
...
static esp_err_t webhook_post_json(const char *url, const char *body,
  ...
  if (err == ESP_OK) {
    ...
    int read_len = esp_http_client_read_response(client, out_snippet, out_snippet_len - 1);
    if (read_len >= 0 && out_snippet) {
        out_snippet[read_len] = '\0';
