...
#define GPIO_OUTPUT_PIN_SEL                                                    \
  ((1ULL << CONNECTED_LED_GPIO_NUM) | (1ULL << ACTIVE_LED_GPIO_NUM) |          \
   (1ULL << PWR_LED_GPIO_NUM))

static QueueHandle_t xMsg_Tx_Queue, xMsg_Rx_Queue, xmsg_ws_tx_queue,
    xmsg_ble_tx_queue, xmsg_obd_rx_queue, xmsg_mqtt_rx_queue;
...
__attribute__((unused)) static bool dbg_net_ready(void) {
  return wifi_network_is_connected();
}
...
static void can_tx_task(void *pvParameters) {
  while (1) {
    ...
    ESP_LOGI(TAG, "----------");
    ESP_LOG_BUFFER_HEXDUMP(TAG, ucTCP_RX_Buffer.ucElement,
                           ucTCP_RX_Buffer.usLen, ESP_LOG_INFO);
    ESP_LOGI(TAG, "----------");
    ...
    if (protocol == SLCAN) {
      ...
      if (real_dash_parse_66(&tx_msg, ucTCP_RX_Buffer.ucElement) == 0) {
        real_dash_parse_44(&tx_msg, ucTCP_RX_Buffer.ucElement,
                           ucTCP_RX_Buffer.usLen);
      }
...
static void precondition_task(void *arg) {
  ...
  while (1) {
    ...
    // is scheduled late.
    ESP_ERROR_CHECK(
        esp_timer_start_once(precondition_timer, PRECONDITION_TICK_PERIOD_US));
  }
}
...
static void precondition_task_start(void) {
  ...
  BaseType_t created = xTaskCreate(
      precondition_task, "precondition_task", PRECONDITION_TASK_STACK_SIZE,
      NULL, PRECONDITION_TASK_PRIORITY, &precondition_task_handle);
...
// Single-bus builds have nothing to bridge, so hardcode parallel there.
  ...
  while (1) {
    ...
    // {
    //     time_old = esp_timer_get_time();
    //     ESP_LOGI(TAG, "heap current: %lu min: %lu max: %lu", free_heap,
    //     min_heap, max_heap);
    // }
    ...
    do {
      ...
      if (config_server_ws_connected() && rx_bus == CAN_BUS_0) {
        ucTCP_TX_Buffer.usLen =
            slcan_parse_frame(ucTCP_TX_Buffer.ucElement, &rx_msg);
        if (config_server_ws_connected()) {
          xQueueSend(xmsg_ws_tx_queue, (void *)&ucTCP_TX_Buffer,
                     pdMS_TO_TICKS(0));
        }
      }
      // TODO: optimize, useless ifs
      if (tcp_port_open() || ble_connected() || mqtt_connected() ||
          protocol == AUTO_PID) {
        ...
        if (protocol == SLCAN) {
          ...
          if (rx_bus == CAN_BUS_0) {
            ucTCP_TX_Buffer.usLen =
                real_dash_set_66(&rx_msg, ucTCP_TX_Buffer.ucElement);
          }
        } else if (protocol == SAVVYCAN) {
          ucTCP_TX_Buffer.usLen =
              gvret_parse_can_frame(ucTCP_TX_Buffer.ucElement, &rx_msg, rx_bus);
        ...
        if (ucTCP_TX_Buffer.usLen != 0) {
          if (tcp_port_open()) {
            ...
            if (xQueueSend(xMsg_Tx_Queue, (void *)&ucTCP_TX_Buffer,
          ...
          if (ble_connected()) {
            xQueueSend(xmsg_ble_tx_queue, (void *)&ucTCP_TX_Buffer,
                       pdMS_TO_TICKS(0));
          }
        }
      }
      if (mqtt_connected() && rx_bus == CAN_BUS_0) {
        ...
        if (mqtt_elm327_log_en == 0) {
          ...
          xQueueSend(xmsg_mqtt_rx_queue, (void *)&mqtt_rx_msg,
                     pdMS_TO_TICKS(0));
        }
      }
...
void app_main(void) {
  ...
  // 128 slots (~10 KB): bigger than upstream 16 in order to ride out wifi/lwip
  // hiccups
  ...
  sprintf((char *)ble_uid, "WiC_%02x%02x%02x%02x%02x%02x", derived_mac_addr[0],
          derived_mac_addr[1], derived_mac_addr[2], derived_mac_addr[3],
          derived_mac_addr[4], derived_mac_addr[5]);
  sprintf((char *)uid, "%02x%02x%02x%02x%02x%02x", derived_mac_addr[0],
          derived_mac_addr[1], derived_mac_addr[2], derived_mac_addr[3],
          derived_mac_addr[4], derived_mac_addr[5]);
  ...
  //); 		can_init(CAN_500K); 		can_enable(CAN_BUS_0);
  //
  ...
  if (protocol != AUTO_PID) {
    if (config_server_get_port_type() == UDP_PORT) {
      tcp_server_init(port, &xMsg_Tx_Queue, &xMsg_Rx_Queue,
                      CONNECTED_LED_GPIO_NUM, 1);
    } else {
      tcp_server_init(port, &xMsg_Tx_Queue, &xMsg_Rx_Queue,
                      CONNECTED_LED_GPIO_NUM, 0);
    }
  }
  ...
  if (config_server_get_ble_config()) {
    ...
    ble_init(&xmsg_ble_tx_queue, &xMsg_Rx_Queue, CONNECTED_LED_GPIO_NUM, pass,
             &ble_uid[0]);
  }
  ...
  if (esp_ota_get_partition_description(running, &running_app_info) == ESP_OK) {
    ...
    if (sscanf(running_app_info.version, "v%ld.%ld", &firmware_ver_major,
               &firmware_ver_minor) == 2) {
      sprintf(firmware_version, "%ld.%ld", firmware_ver_major,
              firmware_ver_minor);
  ...
  // DEBUG_LOGI("INIT", "debug_logs initialized (UDP %s:%d) waiting for WiFi",
  // DEBUG_LOGS_UDP_DEST_IP, DEBUG_LOGS_UDP_PORT);`
}
