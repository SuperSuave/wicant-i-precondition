...
## "CAN Do" Rules ("If This CAN Message, Then Play That CAN Message")

"CAN Do" expands Automate into a reactive, event-driven, and scheduled automation engine for WiCAN.

### 1. Triggers ("IF THIS")
- **CAN Message Trigger**:
  - Source Bus (`CAN_BUS_0` / `CAN_BUS_1`).
  - CAN ID (11-bit standard or 29-bit extended hex).
  - Exact payload match, bitmask matching (`(Data & Mask) == Value`), or math expressions (e.g., `[B0:B1] > 3000`).
- **Clock & Schedule Triggers**:
  - Time of day (`HH:MM:SS`).
  - Day of week bitmask (`Mon-Sun`).
  - Interval timer (repeating every $N$ seconds/minutes).
  - Cron schedule string support.
- **Hardware & Battery Voltage Triggers**:
  - Vehicle battery voltage thresholds (e.g. `< 11.8V` or `> 13.2V`).

### 2. Actions & Sequences ("THEN THAT")
- **Single Frame or Multi-step CAN Sequences (Macros)**: Play an ordered sequence of CAN frames with configurable inter-frame delays (ms).
- **Secondary Actions**: Emit instant MQTT payload or trigger HTTP Webhook POST.

### 3. Execution Modes & Anti-Duplication
- **One-Shot**: Transmits action frame once per trigger edge; latches until reset frame or timeout.
- **On-State-Change**: Transmits only when incoming CAN payload or expression value changes.
- **Poll & Verify**: Transmits action, then polls/waits for status confirmation frame before latching.
- **Continuous**: Transmits on every match subject to `cooldown_ms`.

### 4. Safety Interlocks & Reverse Engineering Mode
- **Custom Safety Gate Conditions (Example)**: Because CAN ID messages vary across vehicle makes and models, speed or gear safety gates can be configured as compound trigger conditions (e.g. `IF [Vehicle Speed CAN ID == 0] AND [Trigger CAN ID == 0x123]`).
- **Reverse Engineering Mode**: Disables CAN Do rule evaluation during active SavvyCAN/GVRET binary sniffing to guarantee 0% frame drop rate and free 100% CPU budget.
