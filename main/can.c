...
enum bus_state { OFF_BUS = 0, ON_BUS = 1, END_BUS };
...
static const uint32_t can_bitrate_bps[] = {5000,   10000,  20000,  25000,
                                           50000,  100000, 125000, 250000,
...
// from it later (no copy), so every in-flight TX needs to have long enough
// lifetime. can_send() copies the message into a slot from this pool;
// on_tx_done frees it. The pool size doubles as the driver tx_queue_depth, and
// a counting semaphore (taken with the caller's timeout) provides the legacy
...
static void can_state_cache_update(const twai_message_t *msg, can_bus_t bus) {
  ...
  if (xSemaphoreTake(s_can_state_mutex, pdMS_TO_TICKS(10)) != pdTRUE)
    return;
...
static bool can_on_rx_done(twai_node_handle_t handle,
  ...
  // For classic CAN, dlc==len. This will need to be updated if we ever support
  // FD
  item.msg.data_length_code = (rx_frame.header.dlc > TWAI_FRAME_MAX_DLC)
                                  ? TWAI_FRAME_MAX_DLC
                                  : rx_frame.header.dlc;
...
static bool can_on_tx_done(twai_node_handle_t handle,
  ...
  can_tx_slot_t *slot =
      __containerof(edata->done_tx_frame, can_tx_slot_t, frame);
...
static bool can_on_state_change(twai_node_handle_t handle,
...
static void can_recovery_task(void *arg) {
  ...
  while (1) {
    ...
    uint32_t pending =
        __atomic_exchange_n(&can_busoff_pending, 0, __ATOMIC_SEQ_CST);
    for (int bus = 0; bus < CAN_BUS_COUNT; bus++) {
      ...
      ESP_LOGW(TAG,
               "bus %d: bus-off #%lu, recreating node (%lu TX dropped so far)",
...
static void can_apply_filter(can_bus_t bus) {
  ...
  ESP_LOGW(TAG,
           "bus %d: legacy filter 0x%08lX/0x%08lX applied as std-id %03lX mask "
           "%03lX",
...
#endif
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "bus %d: filter config failed: %s", bus,
             esp_err_to_name(err));
  }
}
...
static esp_err_t can_mcp2515_bus_init(void) {
  ...
  if (err != ESP_OK &&
      err != ESP_ERR_INVALID_STATE) // INVALID_STATE = already installed
  {
    return err;
  }
...
static esp_err_t can_bus0_create_node(void) {
  twai_onchip_node_config_t config = {
      ...
      .fail_retry_cnt =
          can_cfg[CAN_BUS_0].auto_tx ? -1 : 0, // -1 = retry forever
...
void can_enable(can_bus_t bus) {
  ...
  err = twai_node_register_event_callbacks(can_node[bus], &cbs,
                                           (void *)(uintptr_t)bus);
  if (err == ESP_OK) {
  ...
  // lost. TODO(ejones): maybe eventually we can split into one rx queue per
  // bus?
...
void can_disable(can_bus_t bus) {
  ...
  // The node is deleted with node_lock held so a concurrent can_send() can
  // never touch a freed handle
  xSemaphoreTake(node_lock[bus], portMAX_DELAY);
  ...
  // The enable bit goes first so new senders fail fast instead of piling up on
  // the lock.
...
void can_set_auto_retransmit(can_bus_t bus, uint8_t flag) {
  ...
  if (bus == CAN_BUS_1 && flag == 0) {
    ESP_LOGW(
        TAG,
        "bus %d: one-shot TX unsupported on MCP2515, keeping auto-retransmit",
        bus);
    return;
  }
#endif
...
void can_init(void) {
  ...
  xTaskCreate(can_recovery_task, "can_recovery", 3072, NULL, 6,
              &can_recovery_task_handle);

  for (int bus = 0; bus < CAN_BUS_COUNT; bus++) {
    node_lock[bus] = xSemaphoreCreateMutex();
    tx_slot_sem[bus] =
        xSemaphoreCreateCounting(CAN_TX_SLOT_COUNT, CAN_TX_SLOT_COUNT);
...
esp_err_t can_receive(twai_message_t *message, can_bus_t *bus,
  ...
  xEventGroupWaitBits(
      s_can_event_group, CAN_ENABLE_BIT_ANY,
  ...
  for (int b = 0; b < CAN_BUS_COUNT; b++) {
    ...
    if (dropped != rx_drop_logged[b]) {
      ...
      if (now - rx_drop_log_us[b] > 1000000) {
        ...
        ESP_LOGW(TAG, "bus %d: rx queue full, %lu frames dropped total", b,
                 dropped);
      }
    }
  }
  ...
  *message = item.msg;
  ...
  can_do_process_rx_frame(&item.msg, (uint8_t)item.bus);
  return ESP_OK;
}
...
esp_err_t can_send(can_bus_t bus, twai_message_t *message,
  ...
  if (!(xEventGroupGetBits(s_can_event_group) & CAN_ENABLE_BIT(bus))) {
    ...
    if (now - tx_drop_log_us[bus] > 1000000) {
      ...
      ESP_LOGW(TAG, "bus %d: down (disabled), %lu frames dropped total", bus,
               tx_drop_count[bus]);
    }
    return ESP_ERR_INVALID_STATE;
  }
  ...
  if (xSemaphoreTake(tx_slot_sem[bus], ticks_to_wait) != pdTRUE) {
    ...
    if (now - tx_drop_log_us[bus] > 1000000) {
      tx_drop_log_us[bus] = now;
      ESP_LOGW(TAG, "bus %d: TX queue full, %lu frames dropped total", bus,
               tx_drop_count[bus]);
    }
    return ESP_ERR_TIMEOUT;
  }
