...
## "CAN Do" Rules ("If This CAN Message, Then Play That CAN Message") - Proposed Architecture

The "CAN Do" feature enhances Automate by allowing event-driven reactive behaviors: when a specific CAN frame or message condition is observed on the bus, WiCAN automatically responds by playing/transmitting a configured CAN frame (or performing secondary actions like sending MQTT/Webhook alerts).

### Trigger ("IF THIS") Specifications
- **Source Bus**: Select `CAN_BUS_0` (OBD-II main) or `CAN_BUS_1` (secondary/custom).
- **CAN ID & ID Type**: Standard 11-bit or Extended 29-bit CAN ID (hex string, e.g., `0x2C0` or `0x18DAF110`).
- **Data Pattern Matching**:
  - **Data Hex Match**: Optional exact or wildcard payload match (e.g. `02 01 05 * * * * *`).
  - **Mask & Value**: Bitwise masking on raw data bytes (`(Data & Mask) == Value`).
  - **Expression Condition**: Custom mathematical comparison using the expression parser (e.g., `[B0:B1] > 3000` or `B2 == 0x01`).
- **Trigger Controls**:
  - **Debounce / Cooldown (ms)**: Prevents bus flooding by requiring a minimum time interval before re-triggering.
  - **Trigger Count / Mode**: Continuous vs single-shot trigger mode.

### Action ("THEN THAT") Specifications
- **Target Bus**: Select output bus (`CAN_BUS_0` or `CAN_BUS_1`).
- **CAN Frame Transmission**:
  - **Response CAN ID**: 11-bit or 29-bit CAN ID (e.g., `0x329`).
  - **Response Data Bytes**: 0–8 byte hex payload (e.g., `02 10 03 00 00 00 00 00`).
  - **Dynamic Payload Substitution**: Option to mirror or calculate response bytes from incoming trigger bytes (e.g., `Set Byte 2 to Trigger Byte 3`).
- **Execution Parameters**:
  - **Response Delay (ms)**: Configurable delay before emitting the CAN frame.
  - **Repeat Count & Interval**: Play frame N times with M millisecond spacing (for state toggling or wakeups).
- **Secondary Actions**:
  - **MQTT Alert**: Emits an instant payload to an MQTT topic when the rule triggers.
  - **Webhook Notification**: Emits an HTTP POST payload to a configured endpoint.
